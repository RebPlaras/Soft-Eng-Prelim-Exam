...
class Article extends Database {
    ...
    public function getArticles($id = null) {
        if ($id) {
            $sql = "SELECT articles.*, categories.category_name FROM articles LEFT JOIN categories ON articles.category_id = categories.category_id WHERE article_id = ?";
    ...
    public function requestEdit($id) {
        $sql = "UPDATE articles SET edit_request = 1 WHERE article_id = ?";
        return $this->executeNonQuery($sql, [$id]);
    }

    public function getPendingEditRequests() {
        $sql = "SELECT articles.*, school_publication_users.username FROM articles 
                JOIN school_publication_users ON 
                articles.author_id = school_publication_users.user_id
                WHERE edit_request = 1 ORDER BY articles.created_at DESC";
        return $this->executeQuery($sql);
    }
